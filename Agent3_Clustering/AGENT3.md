...
**⚠ `monthly_saving_rs` is mislabeled — it is a DAILY figure, not monthly.** It is a straight `sum(cost_delta_rs)` per pair, and `dh_fc_mh_assignment.csv`'s `total_cost_rs`/`current_fc_cost_rs`/`cost_delta_rs` are all daily costs (Agent 3's cost model runs on daily demand — see PLAYBOOK.md's existing "`cost_delta_rs` is per day" note). **Always multiply by 30 before presenting any of these three columns to the user as a monthly number** — do not present the raw column value under a "monthly" label. This was caught by presenting the raw column to the user as-is on 2026-07-27 (see PLAYBOOK.md).
...
| `run_phase2` requires `agent3_phase2.py` (+ its own dependency `agent3_pipeline.py`) to be present in `Agent3_Clustering\backend\` | These two files were, by mistake, only ever saved in the pre-rewrite project folder (`Desktop\Claude\Agent 3\backend\`) and never copied into this repo — `run_phase2` failed with `ModuleNotFoundError` until they were copied in (2026-07-27). If `run_phase2` fails the same way again (e.g. after a fresh checkout), copy both files across again. `agent3_phase2.py`'s own top-level `run_phase2_pipeline`/`compute_mh_pair_savings` are dead code from that old fixed-orchestrator project — not called by anything here — safe to ignore. |
| Phase 2 — first real end-to-end validation, 2026-07-27 | Confirmed working on real data (`CENTRALHUB_L_VNS4 → CENTRALHUB_L_GOP1`, see PLAYBOOK.md): `status="ok"`, no issues, correct enumeration ILP, sensible cost deltas. One real bug found and fixed in `agent3.py` itself (not `agent3_phase2.py`): `run_phase2` was importing a module named `agent4_pipeline` that never existed anywhere — changed to `import agent4 as p4`, which is what `agent3_phase2.py` itself already correctly does. No compatibility issues found on the Agent 4 side — the frozen interface in §9/AGENT4.md §9 held up exactly as documented. |
